#include "lru_policy_3_solution.h"
#include <assert.h>

void LruPolicy3Solution::Init(const std::vector<Tenant> &tenants,
                              int total_buffer_size) {
  lru_cache_ = Lru(tenants, total_buffer_size);
  tenant_selection_policy_ = FaultRatioPolicy();
  tenants_ = tenants;
...
void LruPolicy3Solution::AccessPageInJudge(PageAccess page_access) {
...
std::pair<int, bool> LruPolicy3Solution::AccessPage(PageAccess page_access) {
...
void LruPolicy3Solution::UpdateScore(int tenant_id) {
