#include "mq_solution.h"
#include <assert.h>

void MQSolution::Init(const std::vector<Tenant> &tenants,
                      int total_buffer_size) {
  mq_cache_ =
      MQ(tenants, total_buffer_size, num_queues_, lifetime_length_multiplier_,
...
void MQSolution::AccessPageInJudge(PageAccess page_access) {
...
std::pair<int, bool> MQSolution::AccessPage(PageAccess page_access) {

  AccessPageInJudge(page_access);

  auto found = mq_cache_.GetPage(page_access);

  if (found.second) {
    mq_cache_.UpdateAccessHistory(page_access);

    UpdateScore(page_access.tenant_id);
    return {found.first, true};
  } else {

    int available_location = mq_cache_.GetAvailableLocation();
    if (available_location != -1 &&
        cache_used_per_tenant_[page_access.tenant_id - 1] <
            tenants_[page_access.tenant_id - 1].max_buffer_size) {
      mq_cache_.AddPage(page_access, available_location);
    ...
    auto evicted_page = mq_cache_.EvictPage(tenant_id_to_evict);

    --cache_used_per_tenant_[tenant_id_to_evict - 1];

    mq_cache_.AddPage(page_access, evicted_page.buffer_location);
...
void MQSolution::UpdateScore(int tenant_id) {
