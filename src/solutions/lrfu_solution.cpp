#include "lrfu_solution.h"
#include <assert.h>

void LRFUSolution::Init(const std::vector<Tenant> &tenants,
                        int total_buffer_size) {
  lrfu_cache_ = LRFU(tenants, total_buffer_size, lambda_);
...
void LRFUSolution::AccessPageInJudge(PageAccess page_access) {
...
std::pair<int, bool> LRFUSolution::AccessPage(PageAccess page_access) {

  AccessPageInJudge(page_access);

  auto found = lrfu_cache_.GetPage(page_access);

  if (found.second) {
    lrfu_cache_.SetPageAsMostRecentlyUsed(page_access);

    UpdateScore(page_access.tenant_id);
    return {found.first, true};
  } else {

    int available_location = lrfu_cache_.GetAvailableLocation();
    if (available_location != -1 &&
        cache_used_per_tenant_[page_access.tenant_id - 1] <
            tenants_[page_access.tenant_id - 1].max_buffer_size) {
      lrfu_cache_.AddPage(page_access, available_location);
    ...
    auto evicted_page = lrfu_cache_.EvictPage(tenant_id_to_evict);

    --cache_used_per_tenant_[tenant_id_to_evict - 1];

    lrfu_cache_.AddPage(page_access, evicted_page.buffer_location);
...
void LRFUSolution::UpdateScore(int tenant_id) {
