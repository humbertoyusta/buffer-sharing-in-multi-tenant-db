...
Lru::Lru(std::vector<Tenant> tenants, int total_buffer_size)
  ...
  pages_map_.resize(tenants.size());
}

std::pair<int, bool> Lru::GetPage(PageAccess page_access) {
  auto found = pages_map_[page_access.tenant_id - 1].find(page_access.page_id);

  if (found != pages_map_[page_access.tenant_id - 1].end()) {
...
std::pair<int, int> Lru::EvictLeastRecentlyUsedPage(int tenant_id) {
  ...
  pages_map_[tenant_id - 1].erase(page_to_evict.first);
...
void Lru::SetPageAsMostRecentlyUsed(PageAccess page_access) {
  auto found = pages_map_[page_access.tenant_id - 1].find(page_access.page_id);
  int buffer_location = found->second->second;
  assert(found != pages_map_[page_access.tenant_id - 1].end());
  ...
  pages_map_[page_access.tenant_id - 1][page_access.page_id] =
      pages_lists_[page_access.tenant_id - 1].begin();
}

void Lru::AddPage(PageAccess page_access, int buffer_location) {
  assert(pages_map_[page_access.tenant_id - 1].find(page_access.page_id) ==
         pages_map_[page_access.tenant_id - 1].end());
