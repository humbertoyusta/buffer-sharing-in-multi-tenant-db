...
void Lru2Solution::AccessPageInJudge(PageAccess page_access) {
...
std::pair<int, bool> Lru2Solution::AccessPage(PageAccess page_access) {

  AccessPageInJudge(page_access);

  auto found = lru2_cache_.GetPage(page_access);

  if (found.second) {
    lru2_cache_.UpdateAccessHistory(page_access);

    UpdateScore(page_access.tenant_id);
    return {found.first, true};
  } else {

    int available_location = lru2_cache_.GetAvailableLocation();
    if (available_location != -1 &&
        cache_used_per_tenant_[page_access.tenant_id - 1] <
            tenants_[page_access.tenant_id - 1].max_buffer_size) {
      lru2_cache_.AddPage(page_access, available_location);
    ...
    auto evicted_page = lru2_cache_.EvictPage(tenant_id_to_evict);

    --cache_used_per_tenant_[tenant_id_to_evict - 1];

    lru2_cache_.AddPage(page_access, evicted_page.buffer_location);
...
void Lru2Solution::UpdateScore(int tenant_id) {
