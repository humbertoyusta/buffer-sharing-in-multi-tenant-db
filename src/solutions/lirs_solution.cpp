#include "lirs_solution.h"
#include <assert.h>

void LIRSSolution::Init(const std::vector<Tenant> &tenants,
                        int total_buffer_size) {
  lirs_cache_ = LIRS(tenants, total_buffer_size, lirs_buffer_ratio_size_);
...
void LIRSSolution::AccessPageInJudge(PageAccess page_access) {
...
std::pair<int, bool> LIRSSolution::AccessPage(PageAccess page_access) {

  AccessPageInJudge(page_access);

  auto found = lirs_cache_.GetPage(page_access);

  if (found.second) {
    lirs_cache_.UpdateAccessHistory(page_access);

    UpdateScore(page_access.tenant_id);
    return {found.first, true};
  } else {

    int available_location = lirs_cache_.GetAvailableLocation();
    if (available_location != -1 &&
        cache_used_per_tenant_[page_access.tenant_id - 1] <
            tenants_[page_access.tenant_id - 1].max_buffer_size) {
      lirs_cache_.AddPage(page_access, available_location);
    ...
    auto evicted_page = lirs_cache_.EvictPage(tenant_id_to_evict);

    --cache_used_per_tenant_[tenant_id_to_evict - 1];

    lirs_cache_.AddPage(page_access, evicted_page.buffer_location);
...
void LIRSSolution::UpdateScore(int tenant_id) {
