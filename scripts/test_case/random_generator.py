...
class RandomGenerator:
    def __init__(self, test_number: int):
        ...
        self.test_case = import_module(
            f"test_cases.random.test_case_{test_number}.description"
        )
        ...
        self.input_file = f"test_cases/random/test_case_{test_number}/input.txt"
        np.random.seed(0)
    ...
    def generate_test_case(self):
        for tenant in self.test_case.tenants:
            ...
            for page_access_dist_batch in tenant["page_accesses_dist"]:
                ...
                while (
                    current_tenant_page_accesses_sum
                    < prv_length + page_access_dist_batch["length"]
                ):
                    ...
                    if page_access_dist_batch["type"] == "zipfian":
                        ...
                        page_access_dist_batch["type"]
                        == "normal_with_correlated_references"
                    ):
                        batch_pages = generate_normal_with_correlated_references(
                            ...
                            page_access_dist_batch[
                                "first_correlated_reference_min_distance"
                            ],
                            page_access_dist_batch[
                                "first_correlated_reference_max_distance"
                            ],
                            page_access_dist_batch[
                        ...
                        page_access_dist_batch["type"]
                        == "uniform_with_correlated_references"
                    ):
                        ...
                        page_access_dist_batch["type"]
                        == "pareto_with_correlated_references"
                    ):
                        ...
                        page_access_dist_batch["type"]
                        == "zipfian_with_correlated_references"
                    ):
                        ...
                        raise Exception(
                            f"Unknown distribution type: {page_access_dist_batch['type']}"
                        )
                    ...
                    if page_access_dist_batch["interval_type"] == "uniform":
                        ...
                        raise Exception(
                            f"Unknown distribution type: {page_access_dist_batch['interval_type']}"
                        )
                    ...
                    for batch_row in batch:
                        if (
                            ...
                            current_tenant_page_accesses.append(
                                [permutation[batch_row[0] - 1], batch_row[1]]
                            )
                            current_tenant_page_accesses_sum += batch_row[1]
                        else:
                            current_tenant_page_accesses.append(
                                [
                                    permutation[batch_row[0] - 1],
                                    prv_length
                                    + page_access_dist_batch["length"]
                                    - current_tenant_page_accesses_sum,
                                ]
                            )
                            current_tenant_page_accesses_sum = (
        ...
        for page_access in compressed_page_accesses:
            ...
            for _ in range(page_access[2]):
                ...
                if (
                    current_page
                    > self.test_case.tenants[page_access[0] - 1]["database_size"]
                ):
                    current_page = 1
    ...
    def _merge_page_accesses(self):
        if self.merge_policy == "full_merge_and_shuffle":
            ...
            weights = [
                len(tenant["page_accesses"]) for tenant in self.test_case.tenants
            ]
            ...
            while tenant_indices:
                ...
                compressed_page_accesses.append(
                    tenant["page_accesses"][current_positions[tenant_index]]
                )
            ...
            tenants_with_remaining_pages = [
                tenant["tenant_id"] for tenant in self.test_case.tenants
            ]
            while len(tenants_with_remaining_pages) > 0:
                selected_tenant_id = np.random.choice(tenants_with_remaining_pages)
                batch_length = min(
                    np.random.randint(self.min_batch_length, self.max_batch_length + 1),
                    self.test_case.tenants[selected_tenant_id - 1][
                        "page_accesses"
                    ].shape[0]
                    - offsets[selected_tenant_id],
                )
                batch = self.test_case.tenants[selected_tenant_id - 1]["page_accesses"][
                    offsets[selected_tenant_id] : offsets[selected_tenant_id]
                    + batch_length
                ]
