...
class Validator:
    def __init__(self, test_number: int, test_type: str):
    ...
    def validate_total_buffer_size(self):
        assert (
            self.total_buffer_size >= constants.MIN_TOTAL_BUFFER_SIZE
        ), f"Total buffer size must be at least {constants.MIN_TOTAL_BUFFER_SIZE}."
        assert (
            self.total_buffer_size <= constants.MAX_TOTAL_BUFFER_SIZE
    ...
    def validate_tenants(self):
        ...
        assert (
            len(self.tenants) <= constants.MAX_TENANTS
        ), f"There can be at most {constants.MAX_TENANTS} tenants."
        for tenant in self.tenants:
            assert (
                tenant["priority_level"] >= constants.MIN_PRIORITY_LEVEL
            ), f"Priority level must be at least {constants.MIN_PRIORITY_LEVEL}."
            assert (
                tenant["priority_level"] <= constants.MAX_PRIORITY_LEVEL
            ), f"Priority level must be at most {constants.MAX_PRIORITY_LEVEL}."
            assert (
                tenant["minimum_buffer_size"] >= constants.MIN_BUFFER_SIZE
            ), f"Minimum buffer size must be at least {constants.MIN_BUFFER_SIZE}."
            assert (
                tenant["minimum_buffer_size"] <= constants.MAX_BUFFER_SIZE
            ), f"Minimum buffer size must be at most {constants.MAX_BUFFER_SIZE}."
            assert (
                tenant["base_buffer_size"] >= constants.MIN_BUFFER_SIZE
            ), f"Base buffer size must be at least {constants.MIN_BUFFER_SIZE}."
            assert (
                tenant["base_buffer_size"] <= constants.MAX_BUFFER_SIZE
            ), f"Base buffer size must be at most {constants.MAX_BUFFER_SIZE}."
            assert (
                tenant["maximum_buffer_size"] >= constants.MIN_BUFFER_SIZE
            ), f"Maximum buffer size must be at least {constants.MIN_BUFFER_SIZE}."
            assert (
                tenant["maximum_buffer_size"] <= constants.MAX_BUFFER_SIZE
            ), f"Maximum buffer size must be at most {constants.MAX_BUFFER_SIZE}."
            assert (
                tenant["database_size"] >= constants.MIN_DATABASE_SIZE
            ), f"Database size must be at least {constants.MIN_DATABASE_SIZE}."
            assert (
                tenant["database_size"] <= constants.MAX_DATABASE_SIZE
            ), f"Database size must be at most {constants.MAX_DATABASE_SIZE}."
            assert (
                tenant["minimum_buffer_size"] <= tenant["base_buffer_size"]
            ), "Minimum buffer size must be at most base buffer size."
            assert (
                tenant["base_buffer_size"] <= tenant["maximum_buffer_size"]
            ), "Base buffer size must be at most maximum buffer size."
            assert (
                tenant["minimum_buffer_size"] <= tenant["maximum_buffer_size"]
            ), "Minimum buffer size must be at most maximum buffer size."

        assert (
            sum([tenant["minimum_buffer_size"] for tenant in self.tenants])
        ...
        assert (
            ...
            <= constants.MAX_PAGE_ACCESSES_LENGTH
        ), f"Total page accesses length must be at most {constants.MAX_PAGE_ACCESSES_LENGTH}."
    ...
    def validate_input_file(self):
        try:
            with open(self.input_file, "r") as f:
                ...
                tenant_number, total_buffer_size, page_accesses_number = map(
                    int, lines[0].split()
                )
                ...
                assert page_accesses_number == sum(
                    page_access_batch["length"]
                    for tenant in self.tenants
                    for page_access_batch in tenant["page_accesses_dist"]
                ), "Page accesses number mismatch."
                ...
                for i in range(tenant_number):
                    assert (
                        priority_levels[i] == self.tenants[i]["priority_level"]
                ...
                for tenant in range(tenant_number):
                    assert (
                        database_sizes[tenant] == self.tenants[tenant]["database_size"]
                ...
                for tenant in range(tenant_number):
                    ...
                    assert (
                        minimum_buffer_size
                        == self.tenants[tenant]["minimum_buffer_size"]
                    ), "Minimum buffer size mismatch."
                    assert (
                        base_buffer_size == self.tenants[tenant]["base_buffer_size"]
                    ), "Base buffer size mismatch."
                    assert (
                        maximum_buffer_size
                        == self.tenants[tenant]["maximum_buffer_size"]
                ...
                for page_access in range(page_accesses_number):
                    ...
                    assert (
                        tenant_id >= 1 and tenant_id <= tenant_number
                    ), "Tenant ID out of range."
                    assert (
                        page_id >= 1
                        and page_id <= self.tenants[tenant_id - 1]["database_size"]
                    ), f"Page ID {page_id} out of range."
                    page_accesses_per_tenant[tenant_id] += 1

                for tenant in range(1, tenant_number + 1):
                    assert page_accesses_per_tenant[tenant] == sum(
                        page_access_batch["length"]
                        for page_access_batch in self.tenants[tenant - 1][
                            "page_accesses_dist"
                        ]
